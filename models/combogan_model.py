...
#######################################
    ...
    def test(self, output_only=True):
        with torch.no_grad():
            if output_only:
    ...
    def test(self, output_only=False):
        with torch.no_grad():
            if output_only:
                ...
                self.visuals = [self.real_B]
                self.labels = ['real_%d' % self.DB]
    
                # cache encoding to not repeat it everytime
                encoded, _, _, _, _ = self.netG.encode(self.real_B, self.DB)
                for d in range(self.n_domains):
                    if d == self.DB and not self.opt.autoencode:
                        continue
                    fake = self.netG.decode(encoded, d)
                    self.visuals.append( fake )
                    self.labels.append( 'fake_%d' % d )
                    if self.opt.reconstruct:
                        rec = self.netG.forward(fake, d, self.DB)
                        self.visuals.append( rec )
                        self.labels.append( 'rec_%d' % d )
    '''
